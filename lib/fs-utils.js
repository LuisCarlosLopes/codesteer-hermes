...
function computeManagedSignature(targetPath, kind) {
  ...
  if (kind === "skill" || kind === "bootstrap") {
    if (!stats.isSymbolicLink()) {
      // On Windows without symlink support, skills may be copied as directories.
      // Return null so the classifier treats them as "unchanged" (no drift).
      return null;
    }
  ...
  // Normalize line endings to LF before hashing so that signatures are
  // consistent across platforms (Python writes LF; Windows may read CRLF).
