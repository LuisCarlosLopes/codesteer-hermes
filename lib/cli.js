...
function detectPython3() {
  // On Windows, the executable is typically "python" or "py", not "python3"
  const candidates = process.platform === "win32" ? ["python", "py"] : ["python3", "python"];
  for (const candidate of candidates) {
    const result = spawnSync(candidate, ["--version"], { encoding: "utf8" });
    if (!result.error && result.status === 0) {
      // Ensure it's actually Python 3, not Python 2
  ...
  throw new Error(
    "Python 3 não encontrado. Instale Python 3 (https://www.python.org/downloads/) e certifique-se de que está no PATH.",
  );
}

function resolveVenvPython(venvDir) {
  // On Windows the venv layout uses Scripts\python.exe instead of bin/python
