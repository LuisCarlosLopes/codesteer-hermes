...
Obedeça a `_codesteer-hermes/contracts/artifact-contracts.md`: **§1** (cinco seções, nesta ordem), **§2** (referências de evidência) e **§3** (valores `Alta` / `Média` / `Baixa`).

Refinamentos para UI (prevalecem sobre §3 quando mais restritivos):
- `Alta`: tela ou estado observado em runtime, ou rota definida formalmente em código
- `Média`: propósito deduzido de nome de rota + heading + componente de página
- `Baixa`: hipótese por nomenclatura, estrutura parcial ou tela inacessível

Nunca registre propósito com confiança `Alta` sem evidência direta.

### `screen-inventory-raw.md`

Título `# Screen Inventory Raw`. Em **Conteúdo extraído**, tabela: `screen_id | rota_ou_entrypoint | marcador_visual | propósito | evidência | confiança | screenshot`. `screen_id` estável e legível; evidência citando arquivo, rota ou marcador concreto.

### `navigation-graph.md`

Título `# Navigation Graph`. Em **Conteúdo extraído**, linhas do tipo `origem --[ação]--> destino` (grafo adjacente).

### `ui-states-catalog.md` (`L2`/`L3`)

Título `# UI States Catalog`. Em **Conteúdo extraído**, tabela: `screen_id | state | trigger | mudança_visível | evidência | confiança | screenshot`.

Se o nível for `L1` ou não houver runtime, não invente estados; registre indisponibilidade de forma explícita.
